 ...
 */
bool Perfil::editarPerfil() {
	auto *user = new TAD::Users();
	int opcao;
	std::string nome, cidade, senha1, senha2;
	int idade;
	std::cout<<"Digite sua senha: "<<std:endl;
    senha1 = getpass("", true);
    if(Sessao::Session::getUsuarioLogado()->validarLogin(Sessao::Session::getUsuarioLogado()->getAccount(),senha1)){
        ...
        switch (opcao) {
            ...
            case 4:
                do {
                    ...
                    senha1 = getpass("", true);
                    std::cout << "Confirme a nova senha" << std::endl;
                    senha2 = getpass("", true);
                    if((senha1.compare(senha2))!= 0){
                        std::cout << "As senhas não são iguais." << std::endl;
                    }
                }while (senha1.compare(senha2)!= 0)
                    user->editarCampo("password", senha1);
                    Sessao::Session::getUsuarioLogado()->setPassword(senha1);
                    break;
                }
        ...
        cout<<"Senha Inválida"<<std::endl<<"Encerrando Sessão."<<std::endl;
        return(false)
    }
